...
export default function MenuPage() {
  ...
  return (
    <div>
      <h1 className="mb-1 text-xl font-semibold text-slate-900">Menu du jour</h1>
      ...
      {servicesFetch.loading && <LoadingState />}
      ...
      {!servicesFetch.loading && !servicesFetch.error && (
        <div className="mb-6 flex flex-col gap-3">
          ...
          {service?.dishes.map((dish) => (
            <div
              key={dish.id}
              className="flex items-start justify-between gap-3 rounded-xl border border-slate-200 bg-white p-4 transition-all duration-150 hover:-translate-y-0.5 hover:shadow-md"
            >
              <div>
              ...
              <button onClick={() => handleDeleteDish(dish.id)} className="text-xs text-red-600 hover:underline">
                Supprimer
              </button>
            </div>
          ))}
      ...
      <form onSubmit={handleAddDish} className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <Button type="submit" disabled={submitting} className="w-full sm:w-auto">
          {submitting ? 'Ajout…' : 'Ajouter le plat'}
        </Button>
      </form>
    </div>
  );
