...
async function declareIllness(req, res) {
  const { id } = req.params;
  ...
  const result = await db.run(
    'INSERT INTO illness_records (dut1_id, illness_date, note, declared_by) VALUES ($1, $2, $3, $4) RETURNING id',
    [id, illnessDate, note || null, req.user.id]
  );

  const record = await db.get('SELECT * FROM illness_records WHERE id = $1', [result.rows[0].id]);
  res.status(201).json({ illnessRecord: record });
}

async function listIllnessForDut1(req, res) {
  const records = await db.all(
    'SELECT * FROM illness_records WHERE dut1_id = $1 ORDER BY illness_date DESC, id DESC',
    [req.params.id]
  );
  res.json({ illnessRecords: records });
}

async function listIllnessByDate(req, res) {
  const { date } = req.query;
  if (!date) {
    return res.status(400).json({ error: 'date requise (YYYY-MM-DD).' });
  }
